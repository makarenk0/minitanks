...
TileMap::TileMap(std::string FILE, int tileSize, int width, int height, bool randomMap, bool editMode) {
	initMap(FILE, tileSize, width, height, randomMap, editMode);
 
}

void TileMap::initMap(std::string FILE, int tileSize, int width, int height, bool randomMap, bool editMode) {
	...
	overlayTexture.loadFromFile("textures/overlays.png");
...
void TileMap::changeCurrentHealth(int xPoint, int yPoint, int delta)
{
	...
	if (curHealth < 0) {  // if the brick is destroyed, set the texture of ground
		...
		solidBool[yPoint][xPoint] = false;   // set this tile available for player(not solid)
		return;
...
void TileMap::generateMapRandom(){
	...
	for (int i = 0; i < (width / tileSize) * (height / tileSize); i++) {
		tileId = rand() % 3;
		health = rand() % 4;
		if (tileId == 0) {     //this means that if this tile isnt a brick, it couldnt have different health
		...
		randomMap << "(" << i % (width / tileSize) <<","<< std::floor(i / (width/ tileSize))<<","<< tileId <<","<< health <<","<< sol<<",)"<<std::endl;
	}
	randomMap << "end";
	randomMap.close();
}
...
void TileMap::generateMapEmpty()
{
	...
	for (int i = 0; i < (width / tileSize) * (height / tileSize); i++) {
		customMap<< "(" << i % (width / tileSize) << "," << std::floor(i / (width / tileSize)) << "," << 0 << "," << 4 << "," << false << ",)" << std::endl;
	}
	customMap << "end";
	customMap.close();
}
...
void TileMap::openEditWindow() {
	toolsHeight = height;
	tools.create(sf::VideoMode(toolsWidth, toolsHeight), "tools", sf::Style::Titlebar);
	...
	for (int i = 0; i < amountOfTools; i++) {
		...
		toolsPointer[0].position = sf::Vector2f(indent + indent * i + tileSize * i, indent);
		toolsPointer[1].position = sf::Vector2f(indent + indent * i + tileSize * i +tileSize, indent);
		toolsPointer[2].position = sf::Vector2f(indent + indent * i + tileSize * i + tileSize, indent + tileSize);
		toolsPointer[3].position = sf::Vector2f(indent + indent * i + tileSize * i, indent +tileSize);
	...
	for (int i = 0; i < 3; i++) {  //buttons for bricks health
		...
		overlaysMenuPointer[0].position = sf::Vector2f(indent + indent * i + tileSize * i, 2*indent+tileSize);
		overlaysMenuPointer[1].position = sf::Vector2f(indent + indent * i + tileSize * i + tileSize, 2 * indent + tileSize);
		overlaysMenuPointer[2].position = sf::Vector2f(indent + indent * i + tileSize * i + tileSize, 2 * indent + 2*tileSize);
		overlaysMenuPointer[3].position = sf::Vector2f(indent + indent * i + tileSize * i, 2 * indent + 2 * tileSize);
		...
		overlaysMenuPointer[2].texCoords = sf::Vector2f(i * tileSize + tileSize, tileSize);
		overlaysMenuPointer[3].texCoords = sf::Vector2f(i * tileSize, tileSize);

	}
}

bool TileMap::getEditMode(){
	return editMode;
}

void TileMap::setEditMode(bool mode){
	editMode = mode;
}
...
void TileMap::editMap(sf::RenderWindow &mWindow) {
	...
	if (sf::Mouse::isButtonPressed(sf::Mouse::Left))
	{
			...
			if (previousX >= 0 && previousX < width / tileSize && previousY>=0 && previousY < height / tileSize) {
	...
	if (exitEditMode) {
		...
		for (int i = 0; i < (width / tileSize) * (height / tileSize); i++) {
			...
			customMap << "("<< i % (width / tileSize) << "," << std::floor(i / (width / tileSize)) << "," << quadTiles->texCoords.x/tileSize << "," << currentHealth[y][x] << "," << solidBool[y][x] << ",)" << std::endl;
...
void TileMap::setExitEditMode(bool ex) {
	exitEditMode = ex;
}
...
void TileMap::changeTile(int X, int Y) {
	...
	quadOverlay[2].texCoords = sf::Vector2f(healthEdit * tileSize + tileSize, tileSize);
...
void TileMap::drawToolWindow(int winX, int winY) {
	...
	if (sf::Mouse::isButtonPressed(sf::Mouse::Left)) {
		...
		if (x >= toolsMenu[3+4].position.x && x <= toolsMenu[1 +4].position.x) {
			if (y >= toolsMenu[1 +4].position.y && y <= toolsMenu[3 +4].position.y) {
				idEdit = 1;
				healthEdit = 4;
				solEdit = true;
			}
		...
		if (x >= toolsMenu[3 +8].position.x && x <= toolsMenu[1 +8].position.x) {
			if (y >= toolsMenu[1 + 8].position.y && y <= toolsMenu[3 + 8].position.y) {
				idEdit = 2;
				healthEdit = 4;
				solEdit = true;
			}
		} 
		if (x >= toolsMenu[3 + 12].position.x && x <= toolsMenu[1 + 12].position.x) {
			if (y >= toolsMenu[1 + 12].position.y && y <= toolsMenu[3 + 12].position.y) {
				idEdit = 0;
				healthEdit = 5;
				solEdit = false;
			}
		...
		if (x >= overlaysMenu[3+4].position.x && x <= overlaysMenu[1 + 4].position.x) {
			if (y >= overlaysMenu[1 + 4].position.y && y <= overlaysMenu[3 + 4].position.y) {
				idEdit = 1;
				healthEdit = 1;
				solEdit = true;
			}
		}
		if (x >= overlaysMenu[3 + 8].position.x && x <= overlaysMenu[1 + 8].position.x) {
			if (y >= overlaysMenu[1 + 8].position.y && y <= overlaysMenu[3 + 8].position.y) {
				idEdit = 1;
				healthEdit = 2;
				solEdit = true;
			}
